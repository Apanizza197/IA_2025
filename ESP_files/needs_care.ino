...
bool sendData(int soilPct, int lightPct, float temp, bool necesitaRiego, bool isSad)
{
    if (WiFi.status() != WL_CONNECTED) {
        Serial.println("Wi-Fi no conectado. No se hace request.");
        return false;
    }

    HTTPClient http;
    http.begin("https://6rf3e302c1.execute-api.us-east-2.amazonaws.com/dev/data");
    http.addHeader("Content-Type", "application/json");

    String payload = String("{\"soil\":") + soilPct +
                     ",\"light\":" + lightPct +
                     ",\"temp\":" + tempC +
                     ",\"rawSoil\":" + rawSoil +
                     ",\"rawLDR\":" + rawLDR +
    ...
    if (httpResponseCode > 0) {
        ...
        Serial.println("Respuesta de API:");
        Serial.println(response);
    } else {
        Serial.printf("Error de HTTP: %d\n", httpResponseCode);
    }
    http.end();
    return true;
}
...
void loop()
{
    ...
    sendData(soilPct, lightPct, tempC, mustWater, isSad);
